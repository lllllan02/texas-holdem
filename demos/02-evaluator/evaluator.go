...
type HandResult struct {
	Rank      HandRank
	BestCards []core.Card // 构成该牌型的最佳 5 张牌，按重要程度排序
}
...
func Evaluate(cards []core.Card) HandResult {
	if len(cards) < 5 {
...
func evaluate5(cards []core.Card) HandResult {
	// 按点数降序排列
	sorted := make([]core.Card, 5)
	copy(sorted, cards)
	...
	isLowStraight := false
	if !isStraight && sorted[0].Rank == core.RankA && sorted[1].Rank == core.Rank5 &&
		sorted[2].Rank == core.Rank4 && sorted[3].Rank == core.Rank3 && sorted[4].Rank == core.Rank2 {
		isStraight = true
		isLowStraight = true
		// 重新排序，把 A 放到最后，因为这里 A 最小
		sorted = []core.Card{sorted[1], sorted[2], sorted[3], sorted[4], sorted[0]}
	}

	// 统计点数频率
	counts := make(map[core.Rank]int)
	...
	if isFlush && isStraight {
		if bestCards[0].Rank == core.RankA && !isLowStraight {
			rank = RoyalFlush
		} else {
			rank = StraightFlush
		}
...
func getCombinations(cards []core.Card, k int) [][]core.Card {
	var result [][]core.Card
	var backtrack func(start int, current []core.Card)

	backtrack = func(start int, current []core.Card) {
		if len(current) == k {
			combo := make([]core.Card, k)
			copy(combo, current)
